...
class TfPoseEstimator:
    ...
    @staticmethod
    def detect_face(npimg,humans,imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        ...
        for human in humans:
            ...
            parts = [part for idx, part in human.body_parts.items() if part.score > _THRESHOLD_PART_CONFIDENCE]
            part_coords = [(part.x , part.y) for part in parts if
                       part.part_idx in [0, 1, 2, 3, 4, 5, 6, 7, 14, 15, 16, 17]]

            if len(part_coords) < 5:
                continue

            is_nose, part_nose = _include_part(parts, 0)
            is_neck, part_neck = _include_part(parts, 1)
            is_rshoulder, part_rshoulder = _include_part(parts, 2)
            is_relbow , part_relbow = _include_part(parts, 3)
            is_rwrist , part_rwrist = _include_part(parts, 4)
            is_lshoulder, part_lshoulder = _include_part(parts, 5)
            is_lelbow , part_lelbow = _include_part(parts, 6)
            is_lwrist , part_lwrist = _include_part(parts, 7)
            is_reye, part_reye = _include_part(parts, 14)
            is_leye, part_leye = _include_part(parts, 15)
            is_rear, part_rear = _include_part(parts, 16)
            is_lear, part_lear = _include_part(parts, 17)
            ...
            x2 = max([part[0] for part in part_coords])
            y2 = max([part[1] for part in part_coords])
            upartx = [part.x for part in parts if part.part_idx in [0,14,15,16,17]]


            list_nums = [0,1,2,3,4,5,6,7,14,15,16,17]
            partinfo = [(_include_part(parts,val)) for val in list_nums]
            ...
            for i in range(0,4):
                if(partinfo[rlist[i]][0] and partinfo[llist[i]][0] and partinfo[rlist[i]][1].y < partinfo[llist[i]][1].y):
                    check = True
                    break
            for i in range (0,5):
                if(partinfo[ulist[i]][0]==True):
                    checkup = True
                    break
            for i in range (0,5):
                if(partinfo[dlist[i]][0]==True):
                    checkdown = True
                    break
            # if checkup:
                '''
                ...
                #     elif is_rear:
                #         if is_relbow and part_relbow.x < part_rear.x:
                #             x = part_relbow.x
                #             checkdone = True

                #     if checkdone==False and is_rshoulder:
                #         checkdone = True
                #         if is_relbow:
                #             x = 2*part_relbow.x - part_rshoulder.x
                #         elif is_neck:
                #             x = 2.5*part_rshoulder.x - 1.5*part_neck.x
                #         elif is_lshoulder:
                #             x = 2*part_rshoulder.x - part_lshoulder.x
                #         else:
                #             checkdone = False

                #     if checkdone == False and is_nose:
                #         checkdone = True
                #         if is_rear:
                #             x = 5*part_rear.x - 4*part_nose.x
                #         elif is_reye:
                #             x = 8*part_reye.x - 7*part_nose.x
                #         else:
                #             checkdone = False

                #     if checkdone == False and is_reye:
                #         if is_leye:
                #             x = part_reye.x - 2*(part_leye.x-part_reye.x)
                #             checkdone = True
                ...
                #         x2 = part_lelbow.x
                #         if is_lwrist:
                #             x2 = max(x2,part_lwrist.x)
                #     elif is_lshoulder:
                #         checkdonex2 = True
                #         x2 = part_lshoulder.x
                #         if is_lwrist:
                #             x2 = max(x2,part_lwrist.x)
                #     elif is_lwrist and is_neck:
                #         checkdonx2 = True
                #         x2 = part_lwrist.x+0.3*abs(part_lwrist.x-part_neck.x)
                #     elif is_neck:
                #         checkdonex2 = True
                #         if is_leye:
                #             x2 = part_neck.x + 4*(part_leye.x-part_neck.x)
                #         elif is_lear:
                #             x2 = part_neck.x + 2.5*(part_lear.x-part_neck.x)
                #         else:
                #             checkdonex2 = False
                #     if is_nose and checkdonex2 == False:
                #         checkdonex2 = True
                #         if is_lear:
                #             x2 = part_nose.x + 2.5*(part_lear.x-part_nose.x)
                #         elif is_leye:
                #             x2 = part_nose.x + 4*(part_leye.x-part_nose.x)
                #         else:
                #             checkdonex2 = False
                ...
                #         x2 = part_lwrist.x
                #         if is_lelbow:
                #             x2 = (x2+part_lelbow.x)/2
                #         checkdone = True

                #     elif is_lear:
                #         if is_lelbow and part_lelbow.x > part_lear.x:
                #             x2 = part_lelbow.x
                #             checkdone = True

                #     if checkdone==False and is_lshoulder:
                #         checkdone = True
                #         if is_lelbow:
                #             x2 = part_lelbow.x
                #         elif is_neck:
                #             x2 = part_lshoulder.x+0.5*(part_lshoulder.x-part_neck.x)
                #             # print("blah")
                #         elif is_lshoulder:
                #             x2 = part_lshoulder.x
                #         else:
                ...
                #         checkdone = True
                #         if is_lear:
                #             x2 = part_lear.x+(3*part_lear.x - 3*part_nose.x)
                #         elif is_leye:
                #             x2 = 7*part_leye.x - 6*part_nose.x
                #         else:
                #             checkdone = False
                #     if checkdone == False and is_leye:
                #         if is_reye:
                #             x2 = part_leye.x + 2*(part_leye.x-part_reye.x)
                #             checkdone = True
                ...
                #         x = part_relbow.x
                #         if is_rwrist:
                #             x = min(x,part_rwrist.x)
                #     elif is_rshoulder:
                #         checkdonex2 = True
                #         x = part_rshoulder.x
                #     elif is_rwrist and is_neck:
                #         checkdonx2 = True
                #         x = part_rwrist.x-0.3*(part_neck.x-part_rwrist.x)
                #     elif is_neck:
                #         checkdonex2 = True
                #         if is_reye:
                #             x = part_neck.x - 4*(part_neck.x-part_reye.x)
                #         elif is_rear:
                #             x = part_neck.x - 2.5*(part_neck.x-part_rear.x)
                #         else:
                #             checkdonex2 = False
                #     if is_nose and checkdonex2 == False:
                #         checkdonex2 = True
                #         if is_rear:
                #             x = part_nose.x - 2.5*(part_nose.x-part_rear.x)
                #         elif is_reye:
                #             x = part_nose.x - 4*(part_nose.x-part_reye.x)
                #         else:
                #             checkdonex2 = False
                
                # if not checkup and not checkdown:
                #     continue
                
                # if not checkdown and checkup:
                #     if is_neck:
                #         y = y - 1.7*(part_neck.y-y)
                #         y2 = part_nose.y + 0.3*(part_neck.y-y)
                #     else:
                #         if not(is_reye or is_leye) or not(is_lear or is_rear or is_nose):
                #             continue
                #         else:
                #             diff = y2 - y
                #             y,y2 = y-5*diff , y2+5*diff
                # if not checkup and checkdown:
                #     y2 , y = (2*y + y2)/3,y - 2*(y2-y)   
                '''
            ...
            if checkup:
                ...
                y2 , y = (2*y+y2)/3 , y - 1.5*(y2-y)
                # print("BLAH4")

            if checkup and checkdown:
                y , y2 = y - 0.5*(y2-y), (3*y2 + 2*y)/5        
            x = int(max(x*image_w,5))
            y = int(max(y*image_h,5))
            x2 = int(min(x2*image_w,image_w-5))
            y2 = int(min(y2*image_h,image_w-5))
            
            #print(x,y,x2,y2)
            #cv2.rectangle(npimg,(x,y),(x2,y2),(0,255,0),3)
            ret_list.append(((x,y),(x2,y2)))
        return ret_list

    @staticmethod
    def detect_body(npimg,humans,imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        ...
        for human in humans:
            ...
            parts = [part for idx, part in human.body_parts.items() if part.score > _THRESHOLD_PART_CONFIDENCE]
            part_coords = [(part.x , part.y) for part in parts]
            ...
            ret_list_body.append(((x,y),(x2,y2)))
        
        return ret_list_body
